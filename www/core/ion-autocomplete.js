angular.module('ion-autocomplete', []).directive('ionAutocomplete', [
    '$ionicBackdrop', '$ionicScrollDelegate', '$document', '$q', '$parse', '$interpolate', '$ionicPlatform', '$compile', '$templateRequest', '$translate',
    function ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest, $translate) {
        return {
            ...
            controller: ['$attrs', '$timeout', function ($attrs, $timeout) {
                ...
                $timeout(function () {
                    controller.placeholder = $translate.instant(valueOrDefault(controller.placeholder, 'PLACEHOLDER_CLICK_TO_ADD'));
                    controller.cancelLabel = $translate.instant(valueOrDefault(controller.cancelLabel, 'DONE'));
                    controller.selectItemsLabel = $translate.instant(valueOrDefault(controller.selectItemsLabel, 'LABEL_SELECT_ITEMS'));
                    controller.selectedItemsLabel = $translate.instant(valueOrDefault(controller.selectedItemsLabel, $interpolate("Selected items{{maxSelectedItems ? ' (max. ' + maxSelectedItems + ')' : ''}}:")(controller)));
                });
            ...
            link: function (scope, element, attrs, controllers) {
                ...
                var template = [
                    ...
                    '<input name="autocompl" type="search" class="ion-autocomplete-search" ng-model="viewModel.searchQuery" placeholder="{{viewModel.placeholder}}" maxlength="50" focus-me/>',
                    '</label>',
                    '<div class="error-container last-error-container" ng-show="editForm.$invalid && editForm.autocompl.$touched" ng-messages="editForm.autocompl.$error">',
